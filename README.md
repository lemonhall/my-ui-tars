...
这个项目展示了如何使用UI-TARS Agent进行多轮对话式自动化操作，能够通过自然语言指令控制系统UI执行各种操作任务。

## 功能特点

- **多轮对话操作**：支持连续的对话式操作，通过反馈进行下一步动作
- **自动化UI交互**：点击、双击、拖拽、输入文本、滚动等操作
- **两种运行模式**：
  - 自动反馈模式：自动生成每步操作后的反馈
  - 交互式模式：用户手动提供每步操作的反馈
- **灵活的截图选项**：
  - 实际截图模式：使用真实屏幕截图进行UI分析
  - 缸中脑模式：无需实际截图，适用于测试环境
- **日志级别控制**：
  - 静默模式 (0)：几乎不输出日志
  - 普通模式 (1)：只显示必要信息
  - 详细模式 (2)：显示全部信息，包括模型思考过程
...
```bash
pip install ui-tars-agent pyautogui
```
...
```bash
python example_continuous_actions.py [--mode MODE] [--screenshot SCREENSHOT] [--verbose LEVEL]
```

| 参数 | 说明 | 可选值 | 默认值 |
|------|------|--------|--------|
| `--mode` | 运行模式 | `auto`(自动反馈), `interactive`(交互式) | `auto` |
| `--screenshot` | 是否使用截图 | `true`(使用实际截图), `false`(缸中脑模式) | `true` |
| `--verbose` | 日志详细程度 | `0`(静默), `1`(普通), `2`(详细) | `1` |
...
```bash
python example_continuous_actions.py
```

#### 交互式模式 + 详细日志

```bash
python example_continuous_actions.py --mode interactive --verbose 2
```

#### 自动模式 + 缸中脑 + 静默日志

```bash
python example_continuous_actions.py --screenshot false --verbose 0
```

## 使用流程

1. 运行脚本，指定所需模式和选项
2. 提供初始任务（自动模式下使用预设任务）
3. 系统执行操作并生成反馈
4. 根据反馈继续执行下一步操作
5. 任务完成后生成操作历史摘要
...
- 自动模式下默认执行的任务是："打开微信，给文件传输助手发送一条消息：你好啊"
- 交互式模式中可以使用特殊命令：
  - `quit` 或 `exit`：退出程序
  - `summary`：显示操作历史摘要
  - `auto`：使用自动生成的反馈